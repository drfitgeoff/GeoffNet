...
WITH CLR_FINDER AS 
(SELECT TOTAL_CLAIMED/TOTAL_PAID AS CLR, POLICY_NO FROM
...
(SELECT (TOTAL_DUE - TOTAL_OWED) AS AMOUNT_PAID, 'P' AS TYPE, DT_PAID, POLICY_NO_CONFORMED AS POLICY_NO FROM SH_CLEAN.cp_charges_direct_NEW WHERE DT_PAID BETWEEN @YearOI AND DATE_ADD(@YearOI, INTERVAL 1 YEAR)
UNION ALL
...
SELECT COUNT(*), AVG(CLR), REGION FROM SH_MODEL.LC_MODELLING_SAMPLE_V2 A INNER JOIN CLR_FINDER ON A.POLICY_NO = CLR_FINDER.POLICY_NO WHERE A.MTHS_TO_CANCEL > 11 GROUP BY REGION